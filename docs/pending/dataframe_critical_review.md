...
> **Correction, 2026-09-25.** "FIXED" above covers the *worker-death* hang
> only. A second, unrelated hang was found later: forking from a non-main
> thread could deadlock the workers permanently. `parallel_apply` now refuses
> to fork from anything but the main thread.

...
Generated by `scripts/build_inactive_regions.py` on 2026-09-09 21:00 UTC.
...
variant adds a **6th exclusion** -- assembly N-runs >= 1,000 bp on
main contigs, derived directly from `hg38.fa`
(119,763,735 bp, 3.95% of genome) -- and re-samples
with the identical seed protocol (`default_rng(1337)`, 5000 tiles).

Selected region set for downstream training (per `EXCLUDE_N_GAPS`=False):
`training_tiles.bed`.

| metric | default (`training_tiles.bed`) | nogap (`training_tiles.nogap.bed`) |
|---|---|---|
| tiles | 5000 | 5000 |
| md5 | `8fe67ad46e8c01a9d90f234b9ede4f16` | `a9d80ccd120d36e5a2a6e1025ea8122c` |
| N frac mean | 0.037745 | 0.000000 |
| N frac max | 1.000000 | 0.001282 |
| tiles > 50% N | 189 (3.78%) | 0 (0.00%) |
| tiles > 1% N | 194 (3.88%) | 0 (0.00%) |

Adding the N-gap exclusion collapses the tile N-fraction to ~0 (max
0.001282), removing the 189 effectively-empty
tiles the locked default admits.

**Overlap with default:** 6 of 5000 nogap tiles are
byte-identical to a default tile (0.12%);
872 default tiles overlap any nogap tile.  (Removing N-gaps
...
| `training_tiles.nogap.bed` | 5,000 | `a9d80ccd120d36e5a2a6e1025ea8122c` |

...
def make_qc_plots(stats_df):
    ...
    axarr[0, 0].set_title(
        f"Fwd/Rev Strand Ratios (corr = {round(np.corrcoef(df.T)[0, 1], 3)})"
    )

    stats_df.set_index("gene_name").plot.scatter(
        x="fwd_start_counts_count", y="fwd_start_counts_pred_count", ax=axarr[0, 1]
    )  # , xlim=(0, 20000), ylim=(0, 20000))
    stats_df.set_index("gene_name").plot.scatter(
        x="bkwd_stop_counts_count",
        y="bkwd_stop_counts_pred_count",
        color="orange",
        ax=axarr[0, 1],
    )  # , xlim=(0, 20000), ylim=(0, 20000))
...
def make_tracks(record, sharey=False, smooth=False, max_scale=False):
    ...
    tracks.append(
        GeneTrack(
            "/scratch/karius/annotation/gencode/gencode.v45.basic.annotation.bed.gz",
            region,
        )
    )
    ...
    for base_column, counts, dist in zip(base_columns, all_counts, dists):
        ...
        tracks.append(
            VectorTrack(
                smooth1d(counts, smooth) / smooth1d(dist.mean(), smooth),
                region,
                name="smoothed_ratio",
                color="black",
                alpha=1,
    ...
    tracks.append(
        BedTrack(
            "/home/nboley/src/Ravel/data/repo_data_manifest/annotations/GRCh38_extras/hg38-repeats.sorted.bed.gz",
            region,
        )
    )
    tracks.append(

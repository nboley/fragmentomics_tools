...
class BackgroundModelPredictionsDataFrame(RegionDataFrame):
    def build_stats_df(self):
        return BackgroundModelPredictionsStatsDataFrame(pd.DataFrame(self).progress_apply(calc_stats, axis=1).reset_index())
...
class BackgroundModelModule(L.LightningModule):
    ...
    def predict_from_fasta(self, fa, max_region_size=100000):    
        ...
        for contig, length in zip(fa.references, fa.lengths):
            for start, stop in windowed_range(0, length-region_expansion, max_region_size):
        ...
        for contig, start, stop in tqdm.tqdm(regions):
            windowed_pred[contig].append(self.predict_from_seq(fa.fetch(contig, start, stop)))
        ...
        return pd.DataFrame(dists, columns=['contig', 'start', 'stop'] + [c + "_dist" for c in self.output_columns])
    ...
    def predict_from_rdf_and_sdf(self, rdf, sdf):
        ...
        srdf = SampleAndRegionDataFrame.init_from_rdf_and_sdf(rdf, sdf).attach_fragment_arrays(num_cores=32, min_mapq=10)
        print("Merging fragment arrays (3/5)")
        merged_fas = srdf.groupby("gene_id").progress_apply(
            lambda x: merge_fragment_arrays([fa.drop_duplicate_fragments() for fa in x.fragment_array])
        ).rename('fragment_array')
        print("Building observed arrays (4/5)")

        fas_and_blacklist_regions = pd.DataFrame(rdf).set_index('gene_id')[['blacklist_regions']].join(merged_fas)
        print("Building observed arrays (4/5)")
        
        obs_counts = fas_and_blacklist_regions.progress_apply(
            lambda x: pd.Series(list(FragmentEndpointsDataset.get_targets_from_region_fragment_array(x.fragment_array, self.output_columns, x.blacklist_regions).cpu().detach().numpy())),
            axis=1)
        obs_counts = pd.DataFrame(obs_counts).rename(columns=dict(enumerate(self.output_columns)))

        print("Merging Results (5/5)")
        return BackgroundModelPredictionsDataFrame(rdf.set_index('gene_id').join(obs_counts.join(pred_dists, how='inner')).reset_index(), ref=rdf.ref)